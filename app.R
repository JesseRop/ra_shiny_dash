...
source("ra_libs_raw_dash.R", local = TRUE)
##Creating the ui object for the user interface

ui <- tagList(
  dashboardPage(#skin = "black",
    dashboardHeader(title = "Synovial atlas of Macrophages in RA",
                    titleWidth = 380),
    dashboardSidebar(
      width = 250,
      sidebarMenu(
        ...
        menuItem("Cluster exploration", tabName = "cluster_res", icon = icon("puzzle-piece"),
                 menuSubItem("UMAP", tabName = "all_cluster_res"),
                 menuSubItem("Cluster markers", tabName = "grps_cluster_res")
                 
        ),
        menuItem("Differential expression (DE)", tabName = "ra_de", icon = icon("balance-scale"),
                 menuItem("Gene view", tabName = "vis", icon = icon("braille"),
    ...
    dashboardBody(
      tags$head(
        tags$link(rel = "stylesheet", type = "text/css", href = "amp_custom_dash.css"),
        ...
        # 
        # });"),
        includeHTML("ra_js.htm")
      ),
      # Boxes need to be put in a row (or column)
      tabItems(
        # First tab content
        # source(file.path("ra_introduction_page_dash.R"), local = TRUE)$value,
                    ...
                    tabBox(
                      ...
                      tabPanel("All cells", 
                               wellPanel(
                                 ...
                                 withSpinner(plotOutput("labelled_umap", width = "800px", height = "600px")),
                                 wellPanel(
                                 ...
                                 wellPanel(style = "background:#385A4F",
                                           ...
                                           tags$p(style = "font-family:Arial;color:white",
                                                  paste("Labelling of cell populations based on top cluster markers")
                      ...
                      tabPanel("Group comparison", withSpinner(plotOutput("all_groups", height = "250px")),
  ...
  includeHTML("ra_footer_dash.htm")
)
...
server = function(input, output) {
  ...
  #   filename = function(){paste("input$all_groups_r",'.png',sep='')},
  #     content = function(file){
  #     ggsave(file,plot=all_groups_r())
  #   }
  # )
  output$dwnl_grps <- downloadHandler(
    filename = function(){paste("labelled_umap",input$grps_format,sep="")},
    content = function(file){
      ggsave(file,plot=all_groups_r(), width = input$grps_width, height = input$grps_height, units = "cm", dpi = 300)
    },
    contentType = "image"
  )
  ...
  cluster_markers = reactive({
    ...
    # ra_macrophage_combined_clusters_tables_res_full[[1]][[(as.numeric(input$marker_genes_cluster) + 1)]] %>% rownames_to_column(var = 'genes') %>% mutate_at(vars(matches("p_val|pval") ), ~formatC(., format = "e", digits = 2))
    ra_macrophage_combined_clusters_tables_res_full[[1]][[(as.numeric(input$marker_genes_cluster) + 1)]]  %>% rownames_to_column(var = 'genes') 
  ...
  output$topclgenes <- renderUI({
    req(cluster_markers())
    sliderInput(inputId = "topclgenes_i", label = strong("Number of top cluster markers:"), value = 20, min = 1, max = dim(cluster_markers())[1], step = 1)
  })
  ...
  output$top_conserved_genes = DT::renderDataTable({
    ...
    #   datatable(
    DT::datatable(
      head(cluster_markers(), n = input$topclgenes_i) %>% mutate_at(vars(matches("p_val|pval") ), ~formatC(., format = "e", digits = 2)) %>% dplyr::select(genes,contains(c("avg", "adj"))) %>% mutate_if(is.numeric, ~sprintf("%.3f", .)),
      selection = "single",
      rownames = FALSE,
  ...
  #   filename = function(){paste("input$conserved_markers_umap_r",'.png',sep='')},
  #   content = function(file){
  #     ggsave(file,plot=conserved_markers_umap_r())
  #   }
  # )
  output$dwnl_markers <- downloadHandler(
    filename = function(){paste("labelled_umap",input$markers_format,sep="")},
    content = function(file){
  ...
  output$box_2_2 =renderUI({
    wellPanel(style = "background:#385A4F",
              tags$hr(),
              tags$p(style = "font-family:Arial;color:white",
                     paste("Listing top cluster marker which can subsequently be used in labelling the cluster. Here, markers are genes highly expressed in a cluster as compared to all other clusters irrespective of condition")
              ),
              tags$hr()
    )
  })
  ...
  #   filename = function(){paste("de_stim_vs_ctrl_um_r",'.png',sep='')},
  #   content = function(file){
  #     ggsave(file,plot=de_stim_vs_ctrl_um_r())
  #   }
  # )
  output$dwnl_featurep <- downloadHandler(
    filename = function(){paste("labelled_umap",input$featurep_format,sep="")},
    content = function(file){
      ggsave(file,plot=de_stim_vs_ctrl_um_r(), width = input$featurep_width, height = input$featurep_height, units = "cm", dpi = 300)
    },
    contentType = "image"
  )
  ##Works with the js script on resizing
  # output$dwnl_featurep <- downloadHandler(
  #   filename = "file2.png",
  #   content = function(file){
  #     png(file, width = input$shiny_width, height = input$shiny_height,)
  #     print(de_stim_vs_ctrl_um_r())
  #     dev.off()
  #   }
  # )
  ...
  #   filename = function(){paste("de_stim_vs_ctrl_vp_r",'.png',sep='')},
  #   content = function(file){
  #     ggsave(file,plot=de_stim_vs_ctrl_vp_r())
  #   }
  # )
  output$dwnl_violp <- downloadHandler(
    filename = function(){paste("labelled_umap",input$violp_format,sep="")},
    content = function(file){
  ...
  output$box_1_1 <- renderUI({
    wellPanel(style = "background:#385A4F",
              ...
              tags$p(style = "font-family:Arial;color:white",
                     ...
                     #fluidRow(
                     #column(12,
                     paste("Comparison of", input$de_genes, "expression between", cond," across all clusters using violin plots and umap.")
                     #))
              ),
              tags$hr()
    )  })
  ...
  #   filename = function(){paste("marker_dotplot_r",'.png',sep='')},
  #   content = function(file){
  #     ggsave(file,plot=marker_dotplot_r()())
  #   }
  # )
  output$dwnl_dotp <- downloadHandler(
    filename = function(){paste("labelled_umap",input$dotp_format,sep="")},
    content = function(file){
  ...
  genes_in_de_order = reactive({
    ...
    if(length(unique(umap_cluster_modified_rna()[["FinalClusters"]][["FinalClusters"]])) == length(umap_names)){
      ra_macrophage_combined_de_tables_full[[1]][[as.numeric(match(input$select_cell_type,umap_names))]][[as.numeric(match(input$ra_conds,conds))]]
      
    } else {
      # ra_macrophage_combined_de_tables_full[[1]][[(as.numeric(input$select_cell_type) + 1)]]
      ra_macrophage_combined_de_tables_full[[1]][[(as.numeric(input$select_cell_type) + 1)]][[as.numeric(match(input$ra_conds,conds))]]
  ...
  top_de_g = reactive({
    ...
    genes_in_de_order()  %>% rownames_to_column(var = 'genes') %>% filter(p_val_adj <= 0.05) 
  })
  ...
  output$top_de_genes = DT::renderDataTable({
    ...
    #   datatable(
    DT::datatable(
      top_de_g() %>% mutate_at(vars(matches("p_val|pval") ), ~formatC(., format = "e", digits = 2)) %>% mutate_if(is.numeric, ~sprintf("%.3f", .)),
      # colnames = c(
      #   "Gene",
      #   "scRNA-seq cluster",
      #   "Avg_logFC",
  ...
  cell_type_de = reactive({
    ...
    if(length(unique(umap_cluster_modified_rna()[["FinalClusters"]][["FinalClusters"]])) == length(umap_names)){
      ra_macrophage_combined_de_ggplots_table_full[[1]][[as.numeric(match(input$select_cell_type,umap_names))]]
      
    } else {
      ra_macrophage_combined_de_ggplots_table_full[[1]][[(as.numeric(input$select_cell_type) + 1)]]
    }
  ...
  output$dwnl_scatter <- downloadHandler(
    filename = function(){paste("labelled_umap",input$scatter_format,sep="")},
    content = function(file){
